...
def save_picture(form_picture):
    ...
    output_size = (125, 125) #by importing Pillow package we code this so that our profile pic can resize auto
...
@app.route("/post/new", methods=['GET', 'POST']) #to create our posts
@login_required
def new_post():
    form = PostForm()
    if form.validate_on_submit():
        post = Post(title=form.title.data, content=form.content.data, author=current_user)
        db.session.add(post) #adding our posts to our db
        db.session.commit()
...
def user_posts(username):
    ...
    user = User.query.filter_by(username=username).first_or_404() #get the first username otherwishe throw and 404 not found error (none)
...
def send_reset_email(user):
    ...
    msg = Message('Password Reset Request',
                  sender='renatajarmova@gmail.com',
